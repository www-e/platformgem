...
import { EnrollmentWithHistory, Payment, Course, Lesson } from "@/lib/types/db";
...
export const GET = withErrorHandling(async (_request: NextRequest) => {
  ...
  const monthlyEarnings = courses.reduce((sum, course) => {
    const monthPayments = course.payments.filter((payment: Payment) => new Date(payment.createdAt) >= currentMonth);
    return sum + monthPayments.reduce((monthSum: number, payment: Payment) => monthSum + Number(payment.amount), 0);
  }, 0);
  ...
  const totalViews = allEnrollments.reduce(
    (sum, enrollment) => {
