...
export default async function DashboardPage() {
  ...
  // Fetch courses and the user's enrollments in a single transaction for efficiency
  const [courses, userEnrollments] = await prisma.$transaction([
    ...
    prisma.enrollment.findMany({
      ...
      select: { courseId: true }, // We only need the course IDs for checking
    }),
  ]);

  // Create a simple Set for fast lookups
            ...
            {courses.map((course) => {
              ...
              return (
                <Card key={course.id} className="bg-white/5 backdrop-blur-lg border border-white/10 text-white rounded-2xl flex flex-col card-hover">
                  ...
                  <CardFooter>
                    {isEnrolled ? (
                      <Link href={`/courses/${course.id}`} className="w-full inline-block text-center bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg">
                        عرض الدورة
                      </Link>
                    ) : (
