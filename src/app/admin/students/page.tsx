...
import GradeFilter from "@/components/admin/GradeFilter";
    ...
    <div>
      ...
      <Card className="bg-card">
        ...
        <CardContent>
          <div className="border border-border rounded-lg overflow-x-auto">
            <table className="w-full text-right">
              <thead className="bg-muted/50 border-b border-border">
                <tr>
                  <th className="p-4 font-semibold">الاسم</th><th className="p-4 font-semibold">معرف الطالب</th><th className="p-4 font-semibold">رقم الهاتف</th><th className="p-4 font-semibold">المرحلة</th><th className="p-4 font-semibold text-left">الإجراءات</th>
                </tr>
              </thead>
              <tbody>
                {students.map(student => (
                  <tr key={student.id} className="border-b border-border last:border-b-0 hover:bg-accent/50 transition-colors">
                    <td className="p-4">{student.name}</td><td className="p-4 font-mono text-muted-foreground">{student.studentId}</td><td className="p-4 font-mono text-muted-foreground" dir="ltr">{student.phone}</td><td className="p-4">{gradeMap[student.grade]}</td><td className="p-4 text-left"><Button asChild variant="outline" size="sm"><Link href={`/admin/students/${student.id}`}>عرض التفاصيل</Link></Button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {students.length === 0 && (
            <p className="p-8 text-center text-muted-foreground">{query || grade ? "لم يتم العثور على طلاب مطابقين للبحث." : "لم يقم أي طالب بالتسجيل بعد."}</p>
          )}
        </CardContent>
        {totalPages > 1 && (
